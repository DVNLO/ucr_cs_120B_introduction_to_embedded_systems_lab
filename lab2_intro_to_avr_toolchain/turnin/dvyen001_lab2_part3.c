 ...
 *  Output the number of available spaces on port C and sets C7 if full.
 *
...
int main(void) 
{
    ...
    while (1) 
    {
        PORT_C_TMP = SPACES_COUNT;
        for(i = 0; i < SPACES_COUNT; ++i)
        {
            PORT_C_TMP -= get_bit(i, PINA);
        }
        if(PORT_C_TMP == 0x00)  // 0 spaces available -> set bit 7
            PORT_C_TMP = set_bit(7, PORT_C_TMP);
        PORTC = PORT_C_TMP;
    }
    return 1;
}
